...
impl ExternalWatchServiceImpl {
    ...
    fn to_proto_change_type(change_type: docx_storage_core::ExternalChangeType) -> i32 {
        match change_type {
    ...
    async fn start_watch(
        ...
        match self
            ...
            .start_watch(
                ...
                &req.session_id,
                &source,
                req.poll_interval_seconds as u32,
            )
            .await
        {
            Ok(watch_id) => {
        ...
        tokio::spawn(async move {
            loop {
                ...
                for session_id in &session_ids {
                    match watch_backend
                        .check_for_changes(&tenant_id, session_id)
                        .await
                    {
                        Ok(Some(change)) => {
                ...
                // For R2, we poll less frequently since it's HTTP-based
